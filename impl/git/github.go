...
func newGitHub(config config.Config) gitRemote {
	return &gitHub{
		...
		apiUrl: config.Git.BaseUrl + "/api/v3",
	}
}
...
func (gh *gitHub) createRepository(args cli.Args) (string, error) {
	...
	createResp, err := gh.execPost("orgs/"+groupId+"/repos", data, &ghRepo{})
...
func (gh *gitHub) getGroups() ([]gitGroup, error) {
	...
	return funk.Map(*resp.(*[]ghOrg), gh.toGitGroup).([]gitGroup), nil
}
...
func (gh *gitHub) getRepositories() ([]gitRepository, error) {
	...
	funk.ForEach(gh.config.Git.GroupIds, func(groupId string) {
		...
		for {
			...
			resp, nextPage, err := gh.execGet("orgs/"+groupId+"/repos?type=sources&page="+strconv.Itoa(curPage), &[]ghRepo{})
			curPage = nextPage
			if err == nil {
